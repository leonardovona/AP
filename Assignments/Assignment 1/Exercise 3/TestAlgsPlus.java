...
public class TestAlgsPlus {
    ...
    /**
     * Custom exception thrown when there are more than 1 valid encryption or
     * decryption methods for the same algorithm.
     */
    private static class TooManyMethodsException extends Exception {

        public TooManyMethodsException(String s) {
            super(s);
        }
    ...
    public static void main(String[] args) {
        ...
        Class encrypt_annotation, decrypt_annotation; // Encryp and Decrypt annotations Classes

        try {
            encrypt_annotation = ucl.loadClass("crypto.annot.Encrypt"); // Retrieves the Class of the annotation Encrypt
            decrypt_annotation = ucl.loadClass("crypto.annot.Decrypt"); // Retrieves the Class of the annotation Decrypt
        } catch (ClassNotFoundException ex) {
            System.err.println("ERROR: Class " + ex.getMessage() + " not found during annotation loading");
            return;
        }
        ...
        for (File f : (new File("crypto/algos")).listFiles()) { // Loop through the .class files in the folder crypto/algos
            if (f.getName().endsWith(".class")) {
                ...
                try {
                    ...
                    Method encryption_method = retrieve_method(alg, encrypt_annotation);  // Retrieves the encryption method
                    Method decryption_method = retrieve_method(alg, decrypt_annotation);  // Retrieves the decryption method

                    // Verifies for each secret if the algorithm works properly
                    verify_algorithm(encryption_method, decryption_method, alg);
                } catch (NoSuchMethodException | TooManyMethodsException ex) {
     ...
     * method.
     * @param method Can be the class associated to the annotation Encrypt or
     * Decrypt, identifies the method to retrieve.
     ...
     * not been found.
     * @throws TooManyMethodsException if there are multiple valid encryption /
     * decryption methods.
     */
    private static Method retrieve_method(Class alg, Class annotation) throws NoSuchMethodException, TooManyMethodsException {
        Method method = null;
        for (Method m : alg.getDeclaredMethods()) {
            if (m.getAnnotation(annotation) != null // The method has the correct name
                ...
                if (method == null) {   // First occurrence of a valid method
                    method = m;
                } else {    // A valid method was already found
                    throw new TooManyMethodsException("There are more than one methods annotated with @Decrypt");

                }
            }
        }
        if (method != null) {   // A single valid method has been found
            return method;
        }
        // No valid method has been found
